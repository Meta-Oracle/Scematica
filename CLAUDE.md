...
- **Currently observer mode**: polls `scematica-trades.jsonl` for SELL events; does not yet gate buy decisions. `ready_to_advise = true` when `epsilon < 0.5 && replay_size >= batch_size`.
